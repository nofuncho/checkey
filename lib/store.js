...
export const useAppStore = create((set, get) => ({
  ...
  */
  ...
  try {
    ...
    // 3) 🔥 후처리: 여러 할 일 자동 분리 (', · ; | 줄바꿈 | 그리고/및/하고/와/과/랑/이랑/또)
    const needSplit =
    ...
    function normalizePiece(raw = '') {
      ...
      // 시간/범위 수식어 제거
      s = s.replace(/^(오늘|내일|모레|이번\s*주|다음\s*주|이번\s*달|다음\s*달)\s*/g, '');

      // 끝에 조사가 붙어 있으면 정리 (예: '축구하고' -> '축구')
      s = s.replace(/(하고|이며|이고|이고요|고|랑|이랑|와|과)\s*$/g, '');

      // 동사화: '축구' → '축구하기', '밥먹' → '밥먹기'
      if (!/(기|하기|먹기|하기)$/.test(s)) {
        // '먹' 같이 동사 어간이면 그대로 '기' 붙이기
        if (/[가-힣]$/.test(s)) s = s + '기';
      }

      // 자연스러운 표현 몇 가지
      s = s.replace(/밥\s*먹기?$/g, '밥먹기');
      s = s.replace(/식사\s*하기?$/g, '식사하기');
    ...
    if (needSplit) {
      ...
      // 연결어까지 포함해서 분리
      const rawParts = String(src)
        .split(/,|·|、|;|\||\n|그리고|및|하고|랑|이랑|와|과|또/g)
        .map((s) => s.trim())
        .filter(Boolean);

      const parts = rawParts.map(normalizePiece).filter(Boolean);
    ...
    // 4) 카드도 messages에 "누적" (kind: 'confirmCard')
    set((s) => ({
      messages: [
        ...s.messages,
